...
function run_greeting()  # Splash screen
{
    clear
    echo ""
...
function run_main()
{
        ...
        if [ "${ARG}" = "vara_only" ]; then
            run_downloadbox86 30_Jan_21 # box86 for wine & RMS/VARA (doesn't install dotnet35sp1)
            ...
            run_downloadbox86 1_Jan_21 #  box86 for wine & dotnet35sp1 (doesn't run RMS/VARA) aiming for commit db5efa89
...
function run_checkpermissions()  # Check to make sure that script is not run as root and that user account has sudo permission.
{
...
function run_installwine()  # Download and install wine
{
